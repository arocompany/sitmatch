...
*/
    ...
    String from2 = " FROM TB_SEARCH_RESULT TSR INNER JOIN (SELECT MIN(tsr_uno) tsr_uno from tb_search_result WHERE (:tsiUno is null or tsi_uno = :tsiUno) GROUP BY tsr_site_url) tsr2 ON tsr.tsr_uno = tsr2.tsr_uno INNER JOIN TB_SEARCH_INFO TSI ON TSR.TSI_UNO = TSI.TSI_UNO LEFT OUTER JOIN TB_SEARCH_JOB TSJ ON TSR.TSR_UNO = TSJ.TSR_UNO LEFT OUTER JOIN " +
            ...
            "FROM TB_MATCH_RESULT WHERE tsi_uno = :tsiUno GROUP BY tsi_uno, tsr_uno, tsj_uno) TMR ON TSR.TSR_UNO = TMR.TSR_UNO LEFT OUTER JOIN TB_USER TU ON TSI.USER_UNO = TU.USER_UNO";
...
*/
    ...
    String selectResultWithJob = "SELECT tsr.* FROM tb_search_result tsr " +
            ...
            " WHERE tsr_state = 0 AND data_stat_cd = 10 AND tsj_status = 11 ORDER BY tsr.tsr_uno LIMIT 100 ";
